...
const TaskModal: React.FC<TaskModalProps> = ({ onClose, task }) => {
  ...
  const handleTaskNameBlur = () => {
    ...
    if (taskName.trim().length === 0) {
      ...
      updateTask({ listId: Number(task.list_id), taskId: Number(task.id), task: { ...task, name: taskName } });
    }
  };
  ...
  const handleSave = () => {
      updateTask({ listId: Number(task.list_id), taskId: Number(task.id), task: { ...task, description: descriptionText } });
  ...
  return (
    <div onClick={onClose} className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
      <div onClick={(event) => event.stopPropagation()} className="bg-slate-200 rounded-lg w-5/12 h-5/6 p-8">
        <div className="flex items-center">
          ...
          {editMode ? (
            ...
            <div onClick={() => setEditMode(true)} className="flex items-center cursor-pointer">
        ...
        <div className="modal-body h-5/6 flex pt-8">
          <div className="w-3/4 pr-4">
            <div className="flex items-center mb-2">
              ...
              <h3 className="text-lg font-bold mb-0" style={{ verticalAlign: 'middle', lineHeight: '1' }}>Description</h3>
            </div>
            {isTextareaVisible ? (
              <div>
                <textarea
                ...
                <div className="flex justify-end mt-2">
                  <button onClick={handleSave} className="mr-2 px-4 py-2 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition duration-300">Save</button>
                  <button onClick={toggleTextareaVisibility} className="px-4 py-2 bg-gray-300 text-black rounded-lg shadow-md hover:bg-gray-400 transition duration-300">Close</button>
                </div>
              </div>
            ) : (
              <a onClick={toggleTextareaVisibility} className="cursor-pointer inline-block px-4 py-2 bg-slate-300 text-black rounded-lg shadow-md hover:bg-slate-400 transition duration-300 w-full h-10">
                Add Description
              </a>
            )}
          </div>
  ...
  <div>
    ...
    <div className="flex flex-col gap-2">
      <button className=" bg-slate-300 text-black rounded-lg shadow-md hover:bg-slate-400 transition duration-300 w-full py-2 px-4 ">
        Move To
      </button>
      <button className=" bg-slate-300 text-black rounded-lg shadow-md hover:bg-slate-400 transition duration-300 w-full h-10">
        Delete
      </button>
    </div>
  </div>
